...
import {
  ...
  AddEventAvailabilityOfTutorMutationVariables,
  Event,
  TableEventsDocument,
  TableEventsQuery,
  TableEventsQueryVariables,
  TutorRegistrationDocument,
  TutorRegistrationMutation,
  TutorRegistrationMutationVariables,
  UserIdDocument,
  UserIdQuery,
...
export default function TutorRegistrationForm() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (eventData.events.length) {
        setEvents(eventData.events.map((e) => ({ ...defaultEvent, ...e })));
      }
      setLoading(false);
    };
    void fetchData();
  ...
  useEffect(() => {
    const selectedIds = Object.keys(rowSelection)
      ...
      .map(Number);
    form.setValue("eventsAvailable", selectedIds, {
      shouldValidate: hasTriedToSubmit,
    });
  }, [rowSelection, form, hasTriedToSubmit]);

  async function onValidSubmit(
    tutorData: z.infer<typeof tutorRegistrationFormSchema>
  ) {
    try {
  ...
  const handleError = (error: string) => {
    if (
      error.includes(
        "Error: constraint failed: UNIQUE constraint failed: users.mail"
      )
    ) {
      form.setError("email", {
        type: "custom",
        message:
          "Diese E-Mail wird bereits verwendet, bitte melde dich an um deine Verfügbarkeiten zu ändern",
      });

      // If the server has problems sending the mail, the user will still be added, should be discussed later
    } else if (
      error.includes("failed to send email:") ||
      error.includes("Error: dial tcp")
    ) {
      finishSubmission();
    } else {
      toast.error(
  ...
  return !submitSuccess ? (
    <div
      className={cn(
        " max-[800px]:max-w-[90vw] max-w-[70vw] flex flex-col items-center"
      )}
    >
      ...
      <p className="mb-6 text-center">
        Es freut uns, dass Du Interesse hast uns beim Vorkurs zu unterstützen.
        Bitte gib uns hier deine Kontaktdaten und die Veranstaltungen bei denen
        du als Tutor:in aushelfen kannst an.
        <br /> Im Anschluss werden wir dir eine E-Mail senden, mit deren Hilfe
        du dich dann im System registrieren kannst.
      </p>
      <Form {...form}>
        <form
        ...
        >
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>
                  Bei diesen Veranstaltungen kann ich helfen
                </FormLabel>
                <FormControl>
                  {loading ? (
    ...
    <SubmissionSuccessPage userMail={form.getValues("email")} />
  );
}
